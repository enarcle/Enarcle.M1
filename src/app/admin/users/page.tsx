      ...
      <div style={{ background: C.bg, minHeight: '100%' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '24px 16px' }}>
          ...
          <div style={{ marginBottom: 24 }}>
            <p style={{ fontSize: 11, color: C.blueLight, fontFamily: 'DM Sans,sans-serif', fontWeight: 600, letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: 4 }}>
              Admin
            </p>
          ...
          <div style={{ position: 'relative', marginBottom: 20 }}>
            ...
            />
            {search && (
              <button onClick={() => setSearch('')} style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: C.textDim }}>
          ...
          {loading ? (
            ...
            <div style={{ borderRadius: 16, overflow: 'hidden', border: `1px solid ${C.border}` }}>
              ...
              {filtered.map((u, i) => (
                <div key={u.id} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1.5fr', gap: 12, padding: '13px 20px', alignItems: 'center', borderBottom: i < filtered.length - 1 ? `1px solid ${C.border}` : 'none', opacity: u.is_suspended ? 0.55 : 1, background: i % 2 === 0 ? 'transparent' : C.hoverBg }}>
              ...
              {filtered.length === 0 && !loading && (
                <div style={{ padding: '48px 20px', textAlign: 'center', color: C.textDim, fontFamily: 'DM Sans,sans-serif', fontSize: 14 }}>
