...
import {
  Bell, Users, Heart, MessageCircle,
...
interface NotifMeta { icon: React.ElementType; color: string }
...
export default function NotificationBell({ userId }: { userId: string }) {
  ...
  // If the notifications table doesn't exist yet, suppress silently
  const [tableReady, setTableReady] = useState(true)

  const loadNotifs = async () => {
    if (!userId || !tableReady) return
    setLoading(true)
    try {
      ...
      // Supabase returns a 42P01 code when the table doesn't exist
      if (error) {
        if (error.code === '42P01' || error.message?.includes('does not exist')) {
          setTableReady(false)  // suppress all future queries
        }
        setLoading(false)
        return
      }
      ...
      // Swallow — never crash the parent page
    }
    setLoading(false)
  }
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Realtime subscription errors are non-fatal
    }
  ...
  // If the table doesn't exist, render a disabled bell with no badge
  // This prevents any crash while the migration is pending
  if (!tableReady) {
    return (
      <div style={{ width: 38, height: 38, borderRadius: 10, border: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.textDim }}>
    ...
    <div ref={panelRef} style={{ position: 'relative' }}>
      ...
      <button
        ...
        style={{
          ...
          color: open ? C.indigoL : C.textDim,
      ...
      {open && (
        <div style={{
          position: 'absolute', right: 0, top: 46, width: 320, maxHeight: 460, borderRadius: 16,
          ...
          <div style={{ overflowY: 'auto', flex: 1 }}>
            {loading ? (
              ...
              <div style={{ textAlign: 'center', padding: 40 }}>
                <Bell style={{ width: 32, height: 32, color: C.textDim, margin: '0 auto 10px', display: 'block' }} />
